    ...
    >
          ...
          {menuItems.map((item, i) => (
            <motion.li key={item.href} custom={i} initial="hidden" animate="visible" variants={itemVariants}>
              <a
                href={item.href}
                onClick={onClose}
                className="text-2xl font-medium text-primary block py-2 border-b border-border transition-colors duration-300 hover:text-primary/80"
              >
                {item.label}
              </a>
            </motion.li>
      ...
      >
        <a
          ...
          className="flex items-center gap-3 text-primary transition-colors duration-300 hover:text-primary/80"
        >
